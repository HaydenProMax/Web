  ...
  return (
    ...
    >
      ...
      <section className="grid gap-6 lg:grid-cols-3 xl:grid-cols-4">
        <ModuleCard title="Tasks" description={`${overview.totalCount} active planner tasks are now being tracked.`} eyebrow="Domain" />
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="font-headline text-3xl">Recent Tasks</h2>
          <span className="text-sm text-foreground/50">Showing {tasks.length} of {overview.totalCount} task records</span>
        </div>
        ...
        {tasks.length > 0 ? (
            ...
            {tasks.map((task) => {
              ...
              return (
                <article key={task.id} className="rounded-[2rem] bg-surface-container-low p-6 shadow-ambient">
                  ...
                  <h3 className="mt-3 font-headline text-2xl text-foreground">{task.title}</h3>
                  <p className="mt-3 text-sm leading-6 text-foreground/70">{task.description || "No description yet."}</p>
          ...
          <div className="rounded-[2rem] bg-surface-container-low p-6 text-sm text-foreground/60 shadow-ambient">
            No planner tasks yet. Create one to start shaping the execution layer.
          </div>
        )}
      </section>
    </ShellLayout>
